...
def main():
    ...
    with open(path.joinpath('workflows', 'ci.yaml').absolute(), 'wt') as w:
        ...
        workflow = workflow_header() + jobs(
            ...
            build_and_test_images(id='build-and-test-rcs', name='Build and Test RCs', needs=['init-workflow'], images=rc_images, tests_per_image=tests_per_image, tests=tests),
            trigger_buildkite_job(id='buildkite-rcs', name='Build and Test GPU RCs (on Builtkite)', needs=['init-workflow'], mode='GPU RCS'),
        )
